...
def get_info_invest(vis: Base, ct_, eval_method: str, temp_1: np.ndarray):
...
def filter(vis: Base, DB_PATH, NAM_ID, target, rate, FOLDER_SAVE, critical_col, eval_method: str, add_after_filename=""):
    ...
    temp_info = get_info_invest(vis, "+0", eval_method, temp_1)
    ...
    with tqdm(total=num_rows) as pbar:
        for i in range(num_rows):
            ...
            ct = temp[1]
            info = get_info_invest(vis, ct, eval_method, temp_1)
            if info is None:
                pbar.update(1)
                continue
            check = True
