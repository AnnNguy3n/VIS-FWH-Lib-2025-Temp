...
import os
import pandas as pd
from PySources.mergeTableDB import create_table, get_tb_names
import numpy as np
...
def filter_unique_formula_value(db_path: str, critical_col: str):
    ...
    list_table = [tb_ for tb_ in curs_ori.fetchall() if tb_[0].startswith("T") and not tb_[0].startswith("TT") and not tb_[0].startswith("TE")]
    print(list_table)
    # raise
